...
ホスティングは無料で、独自ドメイン `awastudio.co.jp` の DNS のみ Route 53 で管理しています。
...
2. **Route 53 に DNS レコードを追加**（`awastudio.co.jp` のホストゾーン）
   - apex (`awastudio.co.jp`) に A レコード（GitHub の固定 IP）:
     ```
     185.199.108.153
     185.199.109.153
     185.199.110.153
